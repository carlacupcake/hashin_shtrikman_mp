...
class Member(BaseModel):
    ...
    def get_general_eff_prop_and_cfs(self, idx = 0): # idx is the index in self.values where category properties begin
        ...
        if phase1 == phase2:
            ...
            '''
            '''
            effective_prop_min = phase1 + phase2_vol_frac / (1/(phase2 - phase1) + phase1_vol_frac/(3*phase1))
            effective_prop_max = phase2 + phase1_vol_frac / (1/(phase1 - phase2) + phase2_vol_frac/(3*phase2))
            '''
            '''
        ...
        if phase1_vol_frac == 0:
            ...
            '''
            '''
            cf_response1_cf_load1 = phase1/effective_prop * (1/phase1_vol_frac * (phase2 - effective_prop)/(phase2 - phase1))**2
            cf_response2_cf_load2 = phase2/effective_prop * (1/phase2_vol_frac * (phase1 - effective_prop)/(phase1 - phase2))**2
            '''
            '''
    ...
    def get_elastic_eff_props_and_cfs(self, idx = 0): # idx is the index in self.values where elastic properties begin
        ...
        if phase1_bulk == phase2_bulk:
            ...
            '''
            '''
            effective_bulk_mod_min = phase1_bulk + phase2_vol_frac / (1/(phase2_bulk - phase1_bulk) + 3*phase1_vol_frac/(3*phase1_bulk + 4*phase1_shear))
            effective_bulk_mod_max = phase2_bulk + phase1_vol_frac / (1/(phase1_bulk - phase2_bulk) + 3*phase2_vol_frac/(3*phase2_bulk + 4*phase2_shear))
            '''
            '''
        ...
        if phase1_shear == phase2_shear:
            ...
            '''
            '''
            effective_shear_mod_min = phase1_shear + phase2_vol_frac / (1/(phase2_shear - phase1_shear) + 6*phase1_vol_frac*(phase1_bulk + 2*phase1_shear) / (5*phase1_shear*(3*phase1_bulk + 4*phase1_shear)))
            effective_shear_mod_max = phase2_shear + phase1_vol_frac / (1/(phase1_shear - phase2_shear) + 6*phase2_vol_frac*(phase2_bulk + 2*phase2_shear) / (5*phase2_shear*(3*phase2_bulk + 4*phase2_shear)))
            '''
            '''
        ...
        if phase1_vol_frac == 0:
            ...
            '''
            '''
            cf_phase2_bulk = 1/phase2_vol_frac * (bulk_mod_eff - phase1_bulk) / (phase2_bulk - phase1_bulk)
            cf_phase1_bulk = 1/phase1_vol_frac * (1 - phase2_vol_frac * cf_phase2_bulk)
            '''
            '''
        ...
        if phase1_vol_frac == 0:
            ...
            '''
            '''
            cf_phase2_shear = 1/phase2_vol_frac * (shear_mod_eff - phase1_shear) / (phase2_shear - phase1_shear)
            cf_phase1_shear = 1/phase1_vol_frac * (1 - phase2_vol_frac * cf_phase2_shear)
            '''
            '''
